...
class RReportBarcodeLabels(models.AbstractModel):
    _name = 'report.dynamic_barcode_ring_labels.rr_barcode_labels'
    _description = "rr_barcode_labels"

    @api.model
    def _get_report_values(self, docids, data=None):
        if not data.get('form'):
            raise UserError(_("Form content is missing, this report cannot be printed."))
        config = self.env.ref('dynamic_barcode_ring_labels.default_ring_barcode_configuration')
        if not config:
